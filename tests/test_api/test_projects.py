...
def test_submit_retrospective_returns_safe_error_message_on_unexpected_failure():
    _projects.clear()
    ...
    _projects[project_id] = {
        ...
        "product_description": "Need die cast supplier",
        "status": "complete",
        "current_stage": "complete",
        "error": None,
        "parsed_requirements": None,
    ...
    with patch("app.api.v1.projects._save_project", new=AsyncMock(side_effect=RuntimeError("secret db traceback"))):
        response = client.post(
            f"/api/v1/projects/{project_id}/retrospective",
            json={"supplier_chosen": "Acme", "overall_satisfaction": 4},
            headers=_auth_headers(),
        )
    ...
    assert "secret db traceback" not in payload["detail"]
