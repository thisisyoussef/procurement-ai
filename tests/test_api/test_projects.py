...
def test_list_projects_multi_keyword_matches_across_title_and_description_any_order():
    _projects.clear()
    _projects["motor-housing"] = {
        "id": "motor-housing",
        "user_id": "00000000-0000-0000-0000-000000000001",
    ...
    response = client.get("/api/v1/projects?q=assembly%20motor", headers=_auth_headers())
    assert response.status_code == 200
    payload = response.json()
    assert [project["id"] for project in payload] == ["motor-housing"]


def test_list_projects_multi_keyword_requires_all_terms():
    _projects.clear()
    _projects["motor-housing"] = {
        "id": "motor-housing",
        "user_id": "00000000-0000-0000-0000-000000000001",
    ...
    response = client.get("/api/v1/projects?q=motor%20titanium", headers=_auth_headers())
    assert response.status_code == 200
