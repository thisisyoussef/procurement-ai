...
def test_dashboard_summary_filters_projects_by_project_id_keyword_case_insensitive():
    projects = get_legacy_project_dict()
    projects["proj-dash-target-001"] = {
        "id": "proj-dash-target-001",
        "user_id": "00000000-0000-0000-0000-000000000001",
        "title": "Run A",
        "product_description": "Need zinc-coated steel fasteners for assembly line.",
        "status": "discovering",
    ...
    projects["proj-dash-other-002"] = {
        "id": "proj-dash-other-002",
        "user_id": "00000000-0000-0000-0000-000000000001",
        "title": "Run B",
        "product_description": "Need premium matte labels.",
        "status": "discovering",
    ...
    response = client.get("/api/v1/dashboard/summary?q=TARGET-001", headers=_auth_headers())
    assert response.status_code == 200
    payload = response.json()
    assert [project["id"] for project in payload["projects"]] == ["proj-dash-target-001"]
...
def test_dashboard_summary_combines_status_and_project_id_query_filters():
    projects = get_legacy_project_dict()
    projects["proj-dash-id-match-discovering"] = {
        "id": "proj-dash-id-match-discovering",
        "user_id": "00000000-0000-0000-0000-000000000001",
        "title": "Run A",
        "product_description": "Need zinc-coated steel fasteners for assembly line.",
        "status": "discovering",
    ...
    projects["proj-dash-id-match-complete"] = {
        "id": "proj-dash-id-match-complete",
        "user_id": "00000000-0000-0000-0000-000000000001",
        "title": "Run B",
        "product_description": "Need zinc-coated steel fasteners for assembly line.",
        "status": "complete",
    ...
    response = client.get(
        "/api/v1/dashboard/summary?status=discovering&q=id-match",
        headers=_auth_headers(),
    )
    assert response.status_code == 200
    payload = response.json()
    assert [project["id"] for project in payload["projects"]] == ["proj-dash-id-match-discovering"]
