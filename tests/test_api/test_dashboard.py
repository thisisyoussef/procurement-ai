...
def test_dashboard_summary_store_unavailable_returns_safe_message():
    with patch("app.api.v1.dashboard.get_dashboard_summary_for_user") as get_summary:
        get_summary.side_effect = StoreUnavailableError("summary db offline details")

        response = client.get("/api/v1/dashboard/summary", headers=_auth_headers())

    assert response.status_code == 503
    assert response.json()["detail"] == DASHBOARD_PROJECT_STORE_UNAVAILABLE_DETAIL
    assert "summary db offline details" not in response.json()["detail"]


def test_dashboard_activity_store_unavailable_returns_safe_message():
    with patch("app.api.v1.dashboard.get_dashboard_activity_for_user") as get_activity:
        get_activity.side_effect = StoreUnavailableError("activity db offline details")

        response = client.get("/api/v1/dashboard/activity", headers=_auth_headers())

    assert response.status_code == 503
    assert response.json()["detail"] == DASHBOARD_PROJECT_STORE_UNAVAILABLE_DETAIL
    assert "activity db offline details" not in response.json()["detail"]


def test_dashboard_contacts_store_unavailable_returns_safe_message():
    with patch("app.api.v1.dashboard.get_dashboard_contacts_for_user") as get_contacts:
        get_contacts.side_effect = StoreUnavailableError("contacts db offline details")

        response = client.get("/api/v1/dashboard/contacts", headers=_auth_headers())

    assert response.status_code == 503
    assert response.json()["detail"] == DASHBOARD_PROJECT_STORE_UNAVAILABLE_DETAIL
    assert "contacts db offline details" not in response.json()["detail"]
