...
def test_dashboard_summary_filters_projects_by_project_id_keyword():
    projects = get_legacy_project_dict()
    projects["proj-dash-gearbox"] = {
        "id": "proj-dash-gearbox",
        "user_id": "00000000-0000-0000-0000-000000000001",
        "title": "Run A",
    ...
    response = client.get("/api/v1/dashboard/summary?q=GEARBOX", headers=_auth_headers())
    assert response.status_code == 200
    payload = response.json()
    assert [project["id"] for project in payload["projects"]] == ["proj-dash-gearbox"]
...
def test_dashboard_summary_combines_status_and_project_id_query_filters():
    projects = get_legacy_project_dict()
    projects["proj-dash-gearbox-discovering"] = {
        "id": "proj-dash-gearbox-discovering",
        "user_id": "00000000-0000-0000-0000-000000000001",
        "title": "Run A",
        "product_description": "Need machined gearbox housings.",
        "status": "discovering",
    ...
    projects["proj-dash-gearbox-complete"] = {
        "id": "proj-dash-gearbox-complete",
        "user_id": "00000000-0000-0000-0000-000000000001",
        "title": "Run B",
        "product_description": "Need machined gearbox housings.",
        "status": "complete",
    ...
    response = client.get(
        "/api/v1/dashboard/summary?status=discovering&q=gearbox",
        headers=_auth_headers(),
    )
    assert response.status_code == 200
    payload = response.json()
    assert [project["id"] for project in payload["projects"]] == [
        "proj-dash-gearbox-discovering"
    ]
