...
- `POST /api/v1/projects/{id}/outreach/follow-up`, `/recompare`, `/auto-start`, and `/check-inbox` now return safe `500` details for unexpected failures (without exposing raw exception strings), while preserving explicit `400` validation errors such as `No parsed quotes to compare`.
