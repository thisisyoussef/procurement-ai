...
- `POST /api/v1/projects/{id}/skip-questions` is idempotent: if clarifications were already skipped and the project is already in `discovering`, repeated calls return `200` (`status: resumed`) instead of `400`.
