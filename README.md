...
- `GET /api/v1/dashboard/summary` attention cards now normalize legacy project status values (case/whitespace) and fall back to `current_stage` when `status` is blank, so clarifying actions still appear for older records.
