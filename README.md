...
- `GET /api/v1/dashboard/contacts` accepts project status filters via repeated params (`?status=discovering&status=complete`) or comma-separated lists (`?status=discovering,complete`), including aliases `active` and `closed`; when set, contacts are scoped to suppliers linked to any projects in those statuses (not only the latest interaction project).
