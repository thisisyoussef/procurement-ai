...
- `GET /api/v1/dashboard/contacts?q=...` supports tokenized case-insensitive matching across contact name, email, phone, website, city, and country (all query tokens must match, in any field), including digit-only phone lookup against formatted numbers (for example `3125550142` matches `+1 (312) 555-0142`), with query filtering applied before result limiting so relevant matches are not dropped.
