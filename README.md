...
- `GET /api/v1/dashboard/contacts` sorts merged contacts by `interaction_count`, then `project_count`, then most recent interaction time, so dashboard follow-up starts with higher-signal suppliers.
