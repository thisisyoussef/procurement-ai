...
- `POST /api/v1/projects/search` now accepts quick-search payloads with `product_description` only (no `title` required) and returns a safe `500` detail (`"Failed to run quick search. Please try again."`) for unexpected failures.
