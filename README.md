...
- `GET /api/v1/dashboard/activity?q=...` applies a case-insensitive keyword filter across activity title, description, project name, event type, and project id for both DB-backed and runtime-fallback feeds (with query filtering applied before final response limiting).
