...
- `POST /api/v1/projects/{id}/outreach/follow-up` now returns a safe `500` detail (`"Failed to generate follow-up drafts. Please try again."`) for unexpected failures, without exposing internal exception strings.
- `POST /api/v1/projects/{id}/outreach/recompare` now preserves `400` validation details (for example `All quoted suppliers are excluded`) and returns a safe `500` detail (`"Failed to refresh comparison with real quotes. Please try again."`) for unexpected failures.
