...
- `POST /api/v1/projects/{id}/outreach/start` now returns a safe `503` detail (`"Outreach service is temporarily unavailable. Please try again."`) when project storage is unavailable, without exposing backend exception text.
...
- `POST /api/v1/projects/{id}/phone/call` and `POST /api/v1/projects/{id}/phone/configure` now return a safe `503` detail (`"Phone service is temporarily unavailable. Please try again."`) when project storage is unavailable, without exposing backend exception text.
