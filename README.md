...
- `GET /api/v1/dashboard/contacts?project_id=...` scopes contacts to a single project owned by the current user; combining `project_id` with `status` applies both filters together.
