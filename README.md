...
- `GET /api/v1/dashboard/contacts?q=...` splits `q` into keyword terms and returns only suppliers matching every term across contact name, email, phone, website, city, or country (case-insensitive), including digit-only phone lookup against formatted numbers (for example `acme 3125550142` matches `Acme` with `+1 (312) 555-0142`), with query filtering applied before result limiting so relevant matches are not dropped.
