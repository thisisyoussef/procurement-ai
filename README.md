...
- `GET /api/v1/dashboard/summary` normalizes legacy status formatting (trim/case and blank-status stage fallback) before generating attention prompts, so clarifying-required cards appear reliably for older projects.
