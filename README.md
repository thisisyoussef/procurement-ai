...
## Search Behavior
- `GET /api/v1/projects?q=...` and `GET /api/v1/dashboard/summary?q=...` match keyword tokens against both project titles and product descriptions (case-insensitive), require all query tokens to be present across either field, and ignore extra whitespace/punctuation in query text.
