...
## Search Behavior
- `GET /api/v1/projects?q=...` and `GET /api/v1/dashboard/summary?q=...` match keywords against both project titles and product descriptions (case-insensitive). Multi-term queries require all terms to match somewhere in title/description text (order-insensitive), so `q=motor aluminum` returns only projects containing both terms.
