...
- `GET /api/v1/dashboard/contacts?q=...` supports multi-keyword matching across supplier name, email, phone, website, city, and country (case-insensitive). Each keyword must match somewhere in the contact record, including digit-only phone lookup against formatted numbers (for example `acme 3125550142` matches `Acme` and `+1 (312) 555-0142`), with query filtering applied before result limiting so relevant matches are not dropped.
