...
- `GET /api/v1/dashboard/activity` and `GET /api/v1/dashboard/summary` now include activity `project_name` enrichment when `project_id` is present, so activity rows stay identifiable even under dashboard card filters.
