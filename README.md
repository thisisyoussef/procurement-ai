...
- `GET /api/v1/dashboard/contacts` accepts project status filters via repeated params (`?status=discovering&status=complete`) or comma-separated lists (`?status=discovering,complete`), including aliases `active` and `closed`; when set, contacts are kept when any associated project matches the selected statuses (with legacy fallback to `last_project_id` when only one project link is available).
