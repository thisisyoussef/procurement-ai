...
## Search Behavior
- `GET /api/v1/projects?q=...` and `GET /api/v1/dashboard/summary?q=...` use tokenized, case-insensitive matching across project titles and product descriptions; all query tokens must match, and punctuation/extra spacing in `q` is ignored.
