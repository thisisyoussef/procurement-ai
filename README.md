...
- `GET /api/v1/dashboard/summary` now scopes `recent_activity` to the same filtered project set when `status` and/or `q` filters are applied, so dashboard cards and activity feed stay aligned.
