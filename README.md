...
- `GET /api/v1/dashboard/contacts` status filters include suppliers linked to any matching project, even when a supplier's most recent interaction belongs to a different project status.
...
- `GET /api/v1/dashboard/activity?q=...` applies a case-insensitive keyword filter across activity event title, description, event type, and project name, with query filtering applied before result limiting.
- `GET /api/v1/dashboard/activity` accepts project status filters via repeated params (`?status=discovering&status=complete`) or comma-separated lists (`?status=discovering,complete`), including aliases `active` and `closed`; when set, activity items are scoped to events from projects in those statuses.
- `GET /api/v1/dashboard/activity?q=...` applies a case-insensitive keyword filter across activity title, description, project name, event type, and project id (with query filtering applied before final response limiting).
...
- `GET /api/v1/dashboard/summary`, `GET /api/v1/dashboard/activity`, `GET /api/v1/dashboard/contacts`, and `POST /api/v1/dashboard/projects/start` now return a safe `503` detail (`"Project data is temporarily unavailable. Please try again."`) when the project store is unavailable, without leaking backend outage details.
