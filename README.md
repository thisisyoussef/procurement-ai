...
- `POST /api/v1/projects/{id}/cancel` and `POST /api/v1/projects/{id}/restart` now use canonical status fallback (`status` -> `current_stage`) for lifecycle guards, so legacy stage-only active runs can be canceled and cannot be restarted while in progress.
