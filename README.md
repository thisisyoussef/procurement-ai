...
- `GET /api/v1/dashboard/summary` scopes `recent_activity` to the same project status filter used for summary cards (`status=...`), including `active` and `closed` aliases, so cards and feed stay aligned.
