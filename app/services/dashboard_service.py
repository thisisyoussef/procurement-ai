...
def _contact_matches_query(contact: dict[str, Any], query: str) -> bool:
    tokens = [token for token in re.split(r"[^a-z0-9]+", query.strip().lower()) if token]
    if not tokens:
        return True
    ...
    for token in tokens:
        if any(token in value for value in searchable):
            continue
        phone_token = re.sub(r"\D", "", token)
        if phone_token and phone_value and phone_token in phone_value:
            continue
        return False
    return True
