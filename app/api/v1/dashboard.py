...
PROJECT_STORE_UNAVAILABLE_DETAIL = "Project data is temporarily unavailable. Please try again."
...
def _raise_store_unavailable_http_503(exc: StoreUnavailableError) -> None:
    logger.warning("Dashboard project store unavailable", exc_info=True)
...
def _normalized_contacts_query_or_422(query_text: str | None) -> str | None:
    cleaned = (query_text or "").strip()
    if not cleaned:
        return None
    if len(cleaned) < DASHBOARD_CONTACT_QUERY_MIN_LENGTH:
        raise HTTPException(
            status_code=422,
            detail=(
                "Contact query must be at least "
                f"{DASHBOARD_CONTACT_QUERY_MIN_LENGTH} characters when provided."
            ),
        )
    return cleaned
...
async def dashboard_summary(
    ...
    try:
        ...
        _raise_store_unavailable_http_503(exc)
