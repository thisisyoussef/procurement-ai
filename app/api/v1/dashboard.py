...
MIN_DASHBOARD_CONTACT_QUERY_LENGTH = 2
...
async def dashboard_contacts(
    ...
    if query_text and len(query_text) < MIN_DASHBOARD_CONTACT_QUERY_LENGTH:
        raise HTTPException(
            status_code=422,
            detail=(
                "Contact query must be at least "
                f"{MIN_DASHBOARD_CONTACT_QUERY_LENGTH} characters."
            ),
        )
    try:
