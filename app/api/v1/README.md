...
- `GET /api/v1/projects` accepts optional `q` for case-insensitive project ID, title, or
  description keyword filtering (example: `?q=coffee`), applied after ownership and optional
  status filtering.
...
- `GET /api/v1/dashboard/summary` accepts optional `q` for case-insensitive dashboard
  project ID, title, or description keyword filtering (example: `?q=coffee`), combinable with
