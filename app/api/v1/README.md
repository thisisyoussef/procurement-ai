...
- `GET /api/v1/projects` accepts optional `q` for case-insensitive project title keyword filtering
  ...
  Matching is tokenized with AND semantics across title + product description, and
  punctuation/extra whitespace in the query is ignored. The `q` value is limited to 120 characters.
...
- `GET /api/v1/dashboard/summary` accepts optional `q` for case-insensitive dashboard
  project-card keyword filtering across title + product description (example: `?q=coffee`),
  combinable with optional `status` filters. Matching is tokenized with AND semantics,
  and punctuation/extra whitespace in the query is ignored. The `q` value is limited to 120 characters.
