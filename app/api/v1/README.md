...
- `GET /api/v1/dashboard/contacts` accepts optional `q` for case-insensitive supplier contact
  ...
  limited to 120 characters. Digit-only phone queries also match formatted phone values
  (example: `q=3125550142` matches `+1 (312) 555-0142`). Query filtering is applied before
  response limiting so relevant matches are preserved.
